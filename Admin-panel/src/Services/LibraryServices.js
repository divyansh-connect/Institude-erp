...
export const getSeats = async () => {
  const response = await apiFetch(`http://localhost:3000/api/library/seats`);
...
export const postAddStudentLibrary = async (addStd) => {
  try {
    const response = await apiFetch(
      `http://localhost:3000/api/library/seats/addstudent`,
      {
        method: "POST",
...
export const getStudentList = async () => {
  try {
    const response = await apiFetch(`http://localhost:3000/api/library/lists`);
...
export const getStudent = async (stdId) => {
  try {
    const response = await apiFetch(
      `http://localhost:3000/api/library/student/${stdId}`,
    );
...
export const putStudentPay = async (stdId, payload) => {
  try {
    const response = await apiFetch(
      `http://localhost:3000/api/library/student/pay/${stdId}`,
      {
        method: "PUT",
...
export const putStdEdit = async (stdId, payload) => {
  try {
    const response = await apiFetch(
      `http://localhost:3000/api/library/student/edit/${stdId}`,
      {
        method: "PUT",
        body: JSON.stringify(payload),
      },
...
export const deleteStd = async (stdId) => {
  try {
    const response = await apiFetch(
      `http://localhost:3000/api/library/student/delete/${stdId}`,
      {
        method: "DELETE",
      },
    );
